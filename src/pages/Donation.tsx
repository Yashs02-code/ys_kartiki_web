  ...
  Swal.fire({
    ...
    html: `
      <div style="text-align: center;">
        ...
        <p style="color: #666;">May Lord Vitthal and Goddess Rakhumai bless you abundantly.</p>
        <p style="margin-top: 15px; font-size: 14px; color: #999;">A confirmation email will be sent to ${formData.email}</p>
      </div>
    `,
    confirmButtonColor: "#d97706",
  });
...
Your contribution helps us build a magnificent temple for future generations
</p>
</div>
</section>
    ...
    <div className={`container mx-auto max-w-6xl ${inView1 ? "animate-slide-up" : "opacity-0"}`}>
      <Card className="p-8 md:p-12 bg-gradient-to-br from-card to-primary/5">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Together, we're building something sacred</p>
        </div>

        <div className="mb-6">
          <div className="flex justify-between mb-2">
            <span className="text-2xl font-display font-bold text-primary">
              ₹{(raisedAmount / 1000).toFixed(1)}k Raised
            </span>
            <span className="text-lg text-muted-foreground">
              Goal: ₹{(targetAmount / 100000).toFixed(0)}L
            </span>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-8">
          <div className="text-center p-6 bg-background/50 rounded-xl">
            ...
            <div className="text-3xl font-display font-bold mb-1">1,245</div>
            <div className="text-sm text-muted-foreground">Generous Donors</div>
          </div>
          <div className="text-center p-6 bg-background/50 rounded-xl">
            <Heart className="w-8 h-8 mx-auto text-primary mb-2" />
            <div className="text-3xl font-display font-bold mb-1">₹{(raisedAmount / 100000).toFixed(1)}L</div>
            <div className="text-sm text-muted-foreground">Total Raised</div>
          </div>
          <div className="text-center p-6 bg-background/50 rounded-xl">
            <Star className="w-8 h-8 mx-auto text-primary mb-2" />
            <div className="text-3xl font-display font-bold mb-1">125</div>
  ...
  <section ref={ref2} className="py-16 px-4">
    <div className={`container mx-auto max-w-6xl ${inView2 ? "animate-fade-in" : "opacity-0"}`}>
      <div className="grid md:grid-cols-2 gap-8">
        {/* Donation Form */}
        <Card className="p-8">
          <h3 className="text-2xl font-display font-bold mb-6">Make a Donation</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Input id="amount" name="amount" type="number" value={formData.amount} onChange={handleChange} min="1" required />
            </div>
            <div>
            ...
            <Button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-primary to-accent text-lg py-6">
              {loading ? "Processing..." : "Submit Donation"}
            </Button>
            <p className="text-xs text-muted-foreground text-center">
              After submitting, you'll receive instructions for completing the donation via UPI/Bank Transfer
            </p>
          </form>
        </Card>
        ...
        <div className="space-y-6">
          <Card className="p-8 text-center">
            ...
            <h3 className="text-2xl font-display font-bold mb-4">Scan to Donate</h3>
            <div className="bg-muted p-6 rounded-xl inline-block mb-4">
              <div className="w-48 h-48 bg-background rounded-lg flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground">UPI: temple@upi</p>
                </div>
              </div>
            </div>
            <p className="text-sm text-muted-foreground">Scan with any UPI app to donate instantly</p>
          </Card>

          <Card className="p-8">
            <h3 className="text-xl font-display font-semibold mb-4">Bank Transfer Details</h3>
            <div className="space-y-3 text-sm">
              <div>
                <p className="text-muted-foreground">Account Name</p>
                <p className="font-semibold">Vitthal-Rakhumai Temple Trust</p>
              </div>
              <div>
          ...
          <Card className="p-6 bg-primary/10 border-primary/20">
            <p className="text-sm text-center text-muted-foreground">
              All donations are tax-exempt under Section 80G. Receipt will be issued for donations above ₹500.
            </p>
          </Card>
        </div>
      </div>
